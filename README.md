...
**컬러**: 라임 `#DAF100`(기본 강조색) / 검정 `#0E0E0E` / 흰 `#FFFFFF`
**강조색 규칙**: 본문·엔딩의 강조 요소(평점 점, 뱃지 필, 표지 그림자, 인셋 테두리, 오버레이 박스 등)는 표지 강조색을 따른다 — 표지가 `outline` + `highlightFill`이면 그 색, 아니면 기본 라임.
**슬라이드**: 1080×1350 (4:5)
...
| 3 | 인셋 사진 + 검정 박스 | 강조색 테두리 인셋 사진 + 검정 오버레이 | 세로 |
| 4 | 인용 후킹형 | 소개글 첫 문장이 사진 위 줄 하이라이트로 커짐 | 세로 |
| 5 | 2단 그리드 | 표지·무드사진 나란히, 줄글 우측 정렬 | 세로 |
| 6 | 폴라로이드 + 강조색 박스 | 기울인 표지 + 강조색 오버레이 | 세로 |
