...
**슬라이드**: 1080×1350 (4:5)
**공통**: 라벨 없음, VOL 없음. 모든 푸터에 `@book_kkurueogi I 완독을 도와주는 의대생`

**강조색**: `cover.highlightFill`을 지정하면 표지뿐 아니라 인트로·본문·자유페이지·엔딩의 라임 자리 전부에 그 색이 적용된다 (미지정 시 기본 라임).
...
- **자동 연동**: Actions가 생성을 마치면 `output` 브랜치에 이 파일이 올라가고, 레이어보드는 열릴 때
  그 파일을 직접 읽어 마지막 게시물을 자동으로 불러온다 (이 저장소가 public이라 가능).
  생성 완료 후 반영까지 몇 분 걸릴 수 있고, 레이어보드의 "최신 게시물" 버튼으로 갱신하면 된다.
- 수동으로도 가능 — Artifacts(`carousel-png`) zip에 같은 파일이 있으니 레이어보드의
  "파일에서 열기"로 선택하면 된다.
